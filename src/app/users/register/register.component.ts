...
export class RegisterForm {
  ...
  constructor(public envService:EnvironmentService, private userRepo: UserService, private router: Router, private account: AccountService){}
    ...
    if(this.password==this.password2) {
      this.userRepo.register(this.username, this.password, function (response) {
        if (response.code) {
          ...
          clearTimeout(self.timeout);
          self.message = "Successfully created account and logged into account [" + response.data.account.data.user + "] with the session code " + response.key;
          self.registerButton = "Success";
          self.account.set(response.data.account, response.key);
          self.account.data.checked=true;
          localStorage.setItem("session", response.key);
          self.router.navigate(['/']);
        }
      })
    }else{
